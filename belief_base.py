...
class BeliefBase:
    ...
    def _run_reorder_queue(self):
        ...
        for belief, order in self._reorder_queue:
            for belief_ in self.beliefs:
                if belief_.formula == belief:
                    belief_.order = order
            
        self._reorder_queue = []

    def _remove_formula(self, formula):
        """
        Removes any beliefs with given formula, regardless of order.
        """
        for belief in self.beliefs:
            if belief.formula == formula:
                self.belief.remove(belief)
    ...
    def iter_by_order(self):
        ...
        Yields:
            Tuples of type (order,  list of beliefs with that orderlist of beliefs with that order).
